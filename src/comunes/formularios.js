...
export const Form_todos = (key, config=null, quitar=null, disabled=null)=>{
    ...
    let Form= config===null ? Ver_Valores().Formularios[key.trim()] : config.Formularios[key.trim()];
    if (Form && quitar!==null){
        let nuevo = []
...
export const Titulos_todos = async(key, config=null)=>{
    let Form= config===null ? [...Ver_Valores().Titulos[key.trim()]] : [...config.Titulos[key.trim()]];
    Form= Form
            ? Form
            : [ 
                {title:'_id',field:'_id'},
    ...
    let nuevo= Form.map(v=>{
        ...
        if (valor.tipo){ 
            if(valor.tipo.indexOf('lista_')!==-1){
                const lista= Ver_Valores().Listas[valor.tipo];
                valor.formato= (dato)=> {
