...
const StyledInputBase = styled(InputBase)(({ theme }) => ({
    ...
    '& .MuiInputBase-input': {
      ...
      paddingLeft: `calc(0.5em + ${theme.spacing(0.2)})`,
        ...
        <div style={{
            ...
            <Inputc style={{...Config.Estilos.Input_fondo ? Config.Estilos.Input_fondo : {}}}>
                ...
                <StyledInputBase
                    ...
                    style={{
                      // marginLeft: props.icono ? `calc(1em + 40})` : -60 ,
                      // marginRight: props.icono ? `calc(1em + 40})` : 5 , 
...
export default function Page(props) {
  ...
  const Mostrar_Resultado=(data, datos)=>{
    const formato = data.formato ? data.formato(datos) : undefined
    return formato ? formato : data.label
  }
  ...
  const renderForm =(valor)=>{
    ...
    return valor.multiples ? (
      <Grid container spacing={1} key={valor.lista[0]+'G'}>
        {valor.lista.map((lis,it) =>
          <Grid item  size="grow" key={'GI-'+it}>
            {renderForm(lis)}
          </Grid>
        )}
      </Grid>
    ): ['avatar', 'Avatar'].indexOf(valor.tipo)!==-1 ?(
        <div key={'avatar-'+valor.name} style={classes.avatar}>
          {valor.disabled
            ? null:
            <input accept="image/*"
          ...
          <label key={'avatar-l-'+valor.name} htmlFor={"icon-button-avatar-"+valor.name}>
            <IconButton key={'avatar-b-'+valor.name} color="primary" aria-label="upload picture" component="span">
              <Avatar alt={valor.label}
                      ...
                      className={valor.estilo ? valor.estilo : classes.large}
                      sx={valor.estilo ? valor.estilo : {...classes.large}}
      ...
      <div key={'imagen-'+valor.name} style={classes.cont_imagen}>
    ...
    />
      ...
      <FormControlLabel
        ...
        sx= {{color:Config ? Config.Estilos.Input_label.color : '#fff', 
              '.MuiFormControlLabel-label':{color:Config ? Config.Estilos.Input_label.color : '#fff'}
            }}
        control={
          <Checkbox
    ...
    ):['mensaje1'].indexOf(valor.tipo)!==-1 ? (
      <Typography variant={valor.variant ? valor.variant : "subtitle2"}>
        { valor.label}
      </Typography>
    ):['mensaje2'].indexOf(valor.tipo)!==-1 ? (
    ...
    ):['label1'].indexOf(valor.tipo)!==-1 ? (
      <div style={{height:'100%'}}>
        {Mostrar_Resultado(valor, values.resultados)}
      </div>
    ):valor.tipo==='notas'
    ? (
        <div style={{width:'100%'}}>
  ...
  return form.length===0 && !values.botones ? (
    ...
    <div className={classes.root} style={datos.estilospage ? {...classes.root,...datos.estilospage} : {...classes.root}} key={'formt'}>
      ...
      {form.map((valor,i) =>
        <div key= {'div'+i} style={classes.item}>
          {renderForm(valor)}
        </div>
      )}
      {props.datos.Agregar
