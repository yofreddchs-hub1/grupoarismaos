...
import {Paginas, Titulo_default, Resultado_encontrados, conexiones, Ver_Valores} from '../../../comunes';
...
class Tabla extends Component {
  ...
  componentDidMount(){
    ...
    if (titulos===undefined && datos && datos.length!==0){
  ...
  componentWillUnmount(){
    const {socket} = Ver_Valores();
    if(Ver_Valores().tipo==='Electron'){
      socket.off('Sincronizado');  
    }
  ...
  AccionD = (valor)=>{
    ...
    if (this.state.props.AccionD){
      this.state.props.AccionD(valor);
    }else{
      // console.log('No se asigno accion');
    }

  }
  AccionColumna= (columna, valor)=>{

    if (this.state.props.AccionColumna){
      this.state.props.AccionColumna(columna, valor);
    }

  }

  Buscar = async(e)=>{
    let {pagina, datos, total, items, cargaporparte, table, ordenar, cantidad, cargacompleta}= this.state;
    this.setState({actualizando:true})
    ...
    if(!cargaporparte){// || Ver_Valores().tipo==='Electron'){
      ...
      //=============================================================para los formatos 
      // if (this.state.props.cargacompleta){
      //   console.log('>>>>>>>>>>>>>>por crgacompleta')
      //   this.state.props.cargacompleta(datos);
      // }
      // setTimeout(() => {
      ...
      timeout=setTimeout(async() => {
        ...
        //=============================================================para los formatos 
        // if (this.state.props.cargacompleta){
        //   //agregado para el listado de contacto no se si cambia algo
        //   console.log('>>>>>>>>>>>>>>por crgacompleta')
        //   this.state.props.cargacompleta(datos, false);
        // }
        paginacion.cantidad=items;
  ...
  render(){
    return (
      <Page  {...this.state.props}
            ...
            Accion={this.Accion}
            AccionD={this.AccionD}
            AccionColumna={this.AccionColumna}
            buscar={this.state.buscar}
            Buscar={this.Buscar}
      />
    )
