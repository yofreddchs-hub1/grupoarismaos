...
import {Form_todos, genera_formulario, Generar_id, Moneda} from '../../../comunes';
...
export default function Tabla(props) {
  ...
  let {titulos, datos, Config, Titulo, acciones, acciones1, actualizando, Accion, AccionD, AccionColumna, paginacion, Cambio} = props;//progreso,
  
  titulos = titulos && titulos.filas ? titulos : titulos && !titulos.filas ? {filas:1, datos:[titulos], values:titulos} : titulos; 
  ...
  const Detalles = (column, row) =>{

    if (column.formato(row)===undefined){
      return ''
    }
    const datos = column.formato((row)); 
    return datos.map(val=>
      <Typography key={Generar_id()} variant="body1" gutterBottom
        style={{color:val==='NO ENCONTRADO EN EL SISTEMA' ? '#a50d00' : datos.length>1 ? '#c0c600' : '#000' }}
      >
        {val}
      </Typography>  
  ...
  const FilasDatos = (datos)=>{
    ...
    return <TableBody>
          {datos.sort(function (a, b) {
            ...
            .map((row, j) => {
              const titulosd = titulos.values//titulos.datos[titulos.datos.length-1];
              return (
                <StyledTableRow 
                      hover role="checkbox" 
                      tabIndex={-1} 
                      ...
                      onClick={()=>Accion(row)}
                      onDoubleClick={()=>AccionD(row)}
                      sx={(theme)=>({
                        ...!row.local && !row.eliminado && !row.bgcolor
                        ?
                          { '&:nth-of-type(odd)': {
                        ...
                        :{},
                        bgcolor:row.bgcolor ? row.bgcolor : row.eliminado ? coloreliminado : row.local ? colorlocal :'',
                        
                      })}
                      title={row.eliminado ? 'Data eliminada' : row.local? 'Esta data no a sido actualizado en el servidor': ''}
                >
                  {titulosd.map((column,i) => {
                    const value = row[column.field];
                    // if (column.tipo && column.tipo==='monto'){
                    //   console.log(column.field, row, column.formato(row))
                   
                    // }
                    if(column.props){ console.log(column.props)} 
                    return (
                      <StyledTableCell key={column.field+i+j} 
                        align={column.align ? column.align : column.type==='number' ? 'right' : 'left'} 
                        {...column.props ? column.props : {}} 
                        sx= {column.fija 
                              ? {...Config.Estilos.Tabla_columna ? Config.Estilos.Tabla_columna : {},  borderColor:'#fff',borderWidth:0.5,position:'-webkit-sticky', position:'sticky', left: 0, zIndex:50}
                              : !row.eliminado ? {} : {color:'#fff', borderColor:'#fff',borderWidth:0.5}
                            }
                        onClick={()=>{
                          AccionColumna(column, row);
                        }}
                      >
                        {column.tipo && column.tipo==='monto' 
                          ...
                          : column.tipo && column.tipo==='foto'
                            ...
                            : column.tipo && column.tipo==='imagen'
                              ...
                              : column.tipo && column.tipo==='detalles' 
                                ? <div style={{ }}>
                                    {
                                     Detalles(column, row)
                                    }
    ...
    <Paper sx={{ width: '100%', height:'100%' }}>
      <AppBar position="static" style={{padding:10, ...Config.Estilos.Tabla_cabezera ? Config.Estilos.Tabla_cabezera : {} }}>
        <Grid container spacing={0.5} justifyContent="center" alignItems="center">
          <Grid item size={window.innerWidth > 750 ? 3 : 4.5}>
          ...
          <Grid item size={window.innerWidth > 750 ? 5.5 : 7.5} align={'left'}>
            {acciones? acciones : props.enformulario ? Seleccion  :null}
          </Grid>
          <Grid item size={window.innerWidth > 750 ? 3.5 : 12}>
      ...
      <TableContainer sx={{ 
        ...
        <Table stickyHeader={true}  aria-label="sticky table">
          <TableHead >
            { titulos
              ? titulos.datos.map((titulosd)=>
                <TableRow key={Generar_id()} sx={{direction:'initial'}}>
                  {titulosd 
                    ? titulosd.map((column,j) => {
                      let ncolumn={}
                      ...
                      return(
                        <TableCell
                          ...
                          style={{ minWidth: column.minWidth, fontSize:16, fontWeight:'bold',
                                    ...
                                    writingMode:column.vertical ? 'vertical-rl' : '',
                                    transform: column.vertical ? 'rotate(180deg)' : '',
                                    ...column.fija ? {position:'-webkit-sticky', position:'sticky', left: 0, zIndex:100}:{}
                                }}
                        >
                          {column.title}
      ...
      {props.sinpaginacion
        ...
        : paginacion && paginacion !== undefined && paginacion.paginas.length > 1 ?
          <div style={{padding:5, backgroundColor:'#000000', ...Config.Estilos.Tabla_titulos ? Config.Estilos.Tabla_titulos : {}}}>
            <Grid container spacing={0.5} justifyContent="center" alignItems="center">
              <Grid item size={window.innerWidth > 750 ? 3 : 0}></Grid>
              <Grid item size={window.innerWidth > 750 ? 6 : 11}>
              ...
              <Grid item size={ window.innerWidth > 750 ? 3 : 0}>
